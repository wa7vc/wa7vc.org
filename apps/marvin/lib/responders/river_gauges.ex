defmodule Marvin.Responders.RiverGauges do
  ...
  defp river_find_station({search_term, cached_data}) do
    {search_term, cached_data, Enum.find(cached_data, fn(station) -> station.site_code == search_term end)}
  end
  ...
  defp river_build_response({_search_term, _cached_data, station})  do
    Station.to_variables_and_values_lines(station)
  end

  defp river_map_data_to_stationlist_lines(cached_data) do
    Enum.map(cached_data, &Station.to_code_and_name_string/1)
  end
end
