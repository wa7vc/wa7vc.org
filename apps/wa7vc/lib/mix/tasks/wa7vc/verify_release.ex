defmodule Mix.Tasks.Wa7vc.VerifyRelease do
  use Mix.Task
  ...
  def run(_) do
    ...
    {desc_str, desc_return_code} = System.cmd("git", ["describe", "--match", "wa7vc*"])
    unless desc_return_code == 0 do
      IO.puts("ERROR: Could not run \"git describe --match wa7vc*\"")
      exit({:shutdown, 1})
    end

    r = ~r/wa7vc@(?<tag_ver>\d+\.\d+\.\d+)(-(?<extra_commits>\d+)-g(?<abbrev_obj_name>\w+))?\n/
